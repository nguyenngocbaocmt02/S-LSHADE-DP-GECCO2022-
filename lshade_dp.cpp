...
Fitness LSHADE_DP::run(double *fitness_memory)
{
  ...
  vector<vector<variable>> success_cr(M);
  vector<vector<variable>> dif_fitness(M);

  vector<vector<variable>> memory_sf;
  vector<vector<variable>> memory_cr;
  for (int i = 0; i < M; i++)
  {
    vector<variable> mem_f_i(memory_size, 0.5);
    memory_sf.push_back(mem_f_i);

    vector<variable> mem_cr_i(memory_size, 0.5);
    memory_cr.push_back(mem_cr_i);
  }
  ...
  while (count_fes < max_num_evaluations && FE_term == -1)
  {
    ...
    for (int target = 0; target < pop_size; target++)
    {
      ...
      mu_cr = memory_cr[op_code][random_selected_period];
    ...
    for (int i = 0; i < pop_size; i++)
    {
      ...
      else if (children_fitness[i] < fitness[i])
      {
        ...
        success_cr[mut_op[i]].push_back(pop_cr[i]);
    ...
    for (int m = 0; m < M; m++)
    {
      if (num_success_params[m] > 0)
      {
        ...
        for (int i = 0; i < num_success_params[m]; i++)
        {
          ...
          memory_cr[m][memory_pos[m]] += weight * success_cr[m][i] * success_cr[m][i];
          temp_sum_cr += weight * success_cr[m][i];
        }

        memory_sf[m][memory_pos[m]] /= temp_sum_sf;

        if (temp_sum_cr == 0 || memory_cr[m][memory_pos[m]] == -1)
          memory_cr[m][memory_pos[m]] = -1;
        else
          memory_cr[m][memory_pos[m]] /= temp_sum_cr;
...
void LSHADE_DP::operateCurrentToPBest1BinWithArchive(const vector<Individual> &pop, Individual child, int &target, int &p_best_individual, variable &scaling_factor, variable &cross_rate, const vector<Individual> &archive, int &arc_ind_count)
{
  int r1, r2;
...
void LSHADE_DP::operatePBest1BinWithArchive(const vector<Individual> &pop, Individual child, int &target, int &p_best_individual, variable &scaling_factor, variable &cross_rate, const vector<Individual> &archive, int &arc_ind_count)
{
  int r1, r2;
...
void LSHADE_DP::operateTarget1BinWithArchive(const vector<Individual> &pop, Individual child, int &target, variable &scaling_factor, variable &cross_rate, const vector<Individual> &archive, int &arc_ind_count)
{
  int r1, r2;
...
void LSHADE_DP::reducePopulationWithSort(vector<Individual> &pop, vector<Fitness> &fitness, vector<int> &stagnation)
{
  int worst_ind;
...
void LSHADE_DP::setSHADEParameters()
{
  arc_rate = g_arc_rate;

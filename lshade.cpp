...
Fitness LSHADE::run(double *fitness_memory)
{
  ...
  //main loop
  while (nfes < max_num_evaluations && FE_term == -1)
  {
    for (int i = 0; i < pop_size; i++)
      sorted_array[i] = i;
    for (int i = 0; i < pop_size; i++)
    ...
    if (nfes >= g_max_num_evaluations * pow(g_problem_size, fit_mem_index / 5.0 - 3.0))
    {
